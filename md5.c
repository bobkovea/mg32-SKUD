...
void MD5_Update(void *ctxBuf, const void *data, size_t size)
{
	MD5_CTX *ctx = (MD5_CTX*)ctxBuf;
	uint32_t saved_lo;
	uint32_t used, free;
...
void MD5_Final(uint8_t *result, void *ctxBuf)
{
	MD5_CTX *ctx = (MD5_CTX*)ctxBuf;
	uint32_t used, free;
...
void MD5_MakeHash(uint8_t *srcArr, uint32_t srcArrSize, uint8_t* destHash)
{
  MD5_CTX context;
  MD5_Init(&context);
  MD5_Update(&context, srcArr, srcArrSize);
